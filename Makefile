...
# 1. 增加新写的 ir.o 和 translate.o
OBJS = $(SRC_DIR)/syntax.tab.o $(SRC_DIR)/lex.yy.o $(SRC_DIR)/tree.o $(SRC_DIR)/semantic.o $(SRC_DIR)/ir.o $(SRC_DIR)/translate.o
...
$(SRC_DIR)/ir.o: $(SRC_DIR)/ir.c $(SRC_DIR)/ir.h $(SRC_DIR)/tree.h
$(SRC_DIR)/translate.o: $(SRC_DIR)/translate.c $(SRC_DIR)/ir.h $(SRC_DIR)/semantic.h $(SRC_DIR)/tree.h

%.o: %.c
	$(CC) $(CFLAGS) -c $< -o $@

# 2. 自动化测试命令：一键运行 test 目录下的两个测试文件
test: $(TARGET)
	@echo "======================================"
...
clean:
	rm -f $(TARGET) $(SRC_DIR)/*.o $(SRC_DIR)/lex.yy.c $(SRC_DIR)/syntax.tab.c $(SRC_DIR)/syntax.tab.h output.ir
